    ...
    for batch_idx, data_temp in enumerate(train_loader):
        ...
        if batch_idx % args.log_interval == 0:
            if batch_idx * len(b_data) == len(train_loader) - 1:
                print('Train Epoch: {} [{}/{} \tLoss: {:.20f}'.format(epoch, batch_idx * len(b_data), len(train_loader),
...
def evaluate():
    ...
    print('Average CPC of test tiles: ',cpc_df.cpc.mean(), ' stdev: ', cpc_df.cpc.std())
...
train_data = [oa for t in pd.read_csv(db_dir + '/processed/train_tiles.csv', header=None, dtype=object)[0].values for oa
              ...
              # 这个地方的问题是因为tileidxxx的索引值是tile_id，但是需要的是GEOID
